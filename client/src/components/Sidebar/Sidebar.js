...
const Sidebar = ({ setProducts, showSidebar, setShowSidebar }) => {
    ...
    const handleCategoryClick = async (category) => {
        try {
            const response = await fetch(`http://localhost:3000/api/products/types/${category}`);
            const data = await response.json();
            setTypes(data.types);
    ...
    const handleTypeClick = async (type) => {
        try {
            const response = await fetch(`http://localhost:3000/api/products/types/${selectedCategory}?type=${type}`);
            const data = await response.json();
            setProducts(data.products);
        } catch (error) {
