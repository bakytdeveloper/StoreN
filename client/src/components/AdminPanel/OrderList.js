...
const OrderList = ({ setShowSidebar }) => {
    ...
    const updateStatus = async (orderId, newStatus) => {
        try {
            const response = await fetch(`http://localhost:5505/api/orders/update-status/${orderId}`, {
                method: 'PUT',
                headers: {
    ...
    const updateCommentsAdmin = async (orderId, commentsAdmin) => {
        try {
            const response = await fetch(`http://localhost:5505/api/orders/update-comments-admin/${orderId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
