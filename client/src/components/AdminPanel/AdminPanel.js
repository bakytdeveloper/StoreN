...
const AdminPanel = ({setShowSidebar}) => {
    ...
    const handleDeleteProduct = async (productId) => {
        try {
            const response = await fetch(`http://localhost:5005/api/admin/products/${productId}`, {
                method: 'DELETE',
                headers: {
    ...
    const handleFormSubmit = async (formData) => {
        try {
            ...
            if (selectedProduct) {
                response = await fetch(`http://localhost:5005/api/admin/products/${selectedProduct._id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                response = await fetch('http://localhost:5005/api/admin/products', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
