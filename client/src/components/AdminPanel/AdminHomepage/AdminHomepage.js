...
const AdminHomepage = () => {
    ...
    const handleAddGenderImage = () => {
        setGenderImages([...genderImages, newGenderImage]);
        setNewGenderImage('');
        axios.post(`${process.env.REACT_APP_API_URL}/api/homepage`, { sliderImages, genderImages: [...genderImages, newGenderImage], promotions: promotion })
    ...
    return (
        <div className="homepage-images">
            ...
            <section>
                ...
                <input
                    type="text"
                    value={newGenderImage}
                    onChange={(e) => setNewGenderImage(e.target.value)}
                    placeholder="Enter gender image URL"
                />
                <button onClick={handleAddGenderImage}>Add Gender Image</button>
                <div>
                    {genderImages.map((image, index) => (
                        <div key={index} style={{ display: 'inline-block', margin: '10px' }}>
                            <img src={image} alt={`Gender ${index}`} style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
                            <button onClick={() => handleRemoveGenderImage(image)}>Remove</button>
                        </div>
                    ))}
                </div>
            </section>
